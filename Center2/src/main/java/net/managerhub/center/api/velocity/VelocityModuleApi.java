...
/**
 * What a Center2 module may do on Velocity.
 *
 ...
 * by Center2 when the module is stopped, when it fails while starting and when
 * the proxy shuts down. That covers event handlers and scheduled tasks. What a
 * module registers directly on {@link #proxy()} is not known to Center2 and has
 ...
 *
 * <p>Center2 brings no MOTD module and no routing module of its own. It only
...
public interface VelocityModuleApi {
     ...
     * Velocity event. Center2 unsubscribes the handler again when the module is
     * stopped.</p>
     *
     ...
     *
     * <p>The same object may only be registered once; Center2 unregisters it
