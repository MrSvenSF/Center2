...
/**
 * Registers and removes the MHCenter2 commands at runtime.
 *
 ...
 *
 * <p>Removal only touches entries that point to a command instance of MHCenter2,
...
public final class CommandRegistry {
    ...
    /**
     * Replaces every currently registered MHCenter2 command by the given list.
     *
    ...
    /**
     * Removes every MHCenter2 command from the server.
     *
    ...
    /** @return the number of currently registered MHCenter2 commands. */
    public int size() {
        return active.size();
    }
     ...
     * once that the name it wants is taken. A name MHCenter2 registered itself is
     ...
     * plain name belongs to MHCenter2 the namespaced form is removed again and
    ...
    private void requirePrimaryThread() {
        if (!Bukkit.isPrimaryThread()) {
            throw new IllegalStateException("MHCenter2 commands may only be changed on the main server thread.");
        }
    }
