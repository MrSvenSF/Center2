...
### Brauche ich MHCenter2 auf Paper und auf Velocity?

Nein, nicht zwingend. MHCenter2 läuft auf einem einzelnen Paper-Server genauso wie
...
Ja. Ein Modul mit `platform=VELOCITY` ist ein vollwertiges MHCenter2-Modul: es wird
...
Ja, mit `platform=BOTH`. MHCenter2 sorgt für Lebenszyklus, Kontext und
...
Ja. Modul-JAR nach `plugins/MHCenter2/Modules/Jars/` legen und
...
### Sind MHCenter2-Module sicher isoliert?

**Nein.** Ein Modul ist normaler Java-Code im selben Prozess wie der Server und
hat dieselben Möglichkeiten wie jeder andere Code dort. MHCenter2 verwaltet den
...
Nein. MHCenter2 hängt von keinem Permission-Plugin ab; die Nodes werden einfach an
...
Nein, standardmäßig nicht. Alle MHCenter2-Permissions stehen auf `op: false`, das
...
Nein. MHCenter2 ergänzt fehlende neue Standardeinträge selbst, hebt
...
verschiedener Versionen. MHCenter2 erfindet dort deshalb keine Version. Der
MHCenter2-Versionsbereich gilt aber auf beiden Plattformen.
...
Nein. MHCenter2 funktioniert ohne sie vollständig: Konfiguration, Menüs, Commands,
...
MHCenter2 läuft lokal weiter. Der Knoten geht offline, versucht es mit wachsender
...
Nein. MHCenter2 liest von dort niemals einen Text und führt ihn als Konsolenbefehl
...
Nein. MHCenter2 öffnet keinen Port und bringt keinen HTTP-Dienst mit. Die einzige
...
einen Serverneustart, und MHCenter2 sagt das auch, statt so zu tun, als sei die
neue Version aktiv.
...
Multiplayer-Menü setzen. MHCenter2 selbst bringt kein MOTD-Modul mit, es macht es
nur möglich. Siehe [Velocity-Module](Development-Velocity.md).

### Gibt es ein Inventory-Sync-Modul?

Nein, und MHCenter2 wird auch keines mitbringen. Die Infrastruktur dafür ist
