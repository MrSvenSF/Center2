...
Die gemeinsame MariaDB ist **optional**. Sie ist ab Werk aus, und alle lokalen
MHCenter2-Funktionen laufen ohne sie weiter. Netzwerk-Actions verwenden dann den
spielerabhängigen Plugin-Messaging-Fallback.

Sie fügt genau eine Sache hinzu, die Plugin-Nachrichten nicht können: einen
MHCenter2-Knoten erreichen, auf dem **niemand online** ist.
...
SQLite-Datenbank, und MHCenter2 spiegelt sie **nicht** in die MariaDB. Es gibt
...
```sql
CREATE DATABASE mhcenter2 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'mhcenter2'@'%' IDENTIFIED BY 'ein-langes-passwort';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE ON mhcenter2.* TO 'mhcenter2'@'%';
FLUSH PRIVILEGES;
```

`CREATE` braucht der Benutzer nur beim ersten Start: MHCenter2 legt seine Tabellen
selbst an. Danach kannst du das Recht wieder entziehen. Mehr Rechte braucht
MHCenter2 nicht – kein `DROP`, kein `SUPER`, kein Zugriff auf andere Schemata.
...
Jeder Knoten braucht seinen eigenen Namen im Netzwerk. Du vergibst ihn, MHCenter2
...
leer** sein – ohne sie startet das Remote-System nicht, und MHCenter2 sagt das im
Log.
...
Netzwerk nicht auseinanderzuhalten. MHCenter2 erkennt das über die Runtime-ID, die
...
* MHCenter2 bleibt aktiv, lokal ändert sich nichts.
...
Für MHCenter2 1.0.0 wurde eine echte MariaDB-11.8-Instanz zusammen mit Velocity
und zwei Paper-Servern ausgeführt. Heartbeats, Reloads mit und ohne Spieler,
eine Paper-zu-Paper-Modul-Action und der gemeinsame Modul-Storage mit atomarem
`take()` waren erfolgreich. Der vollständige Umfang steht unter
...
## Was MHCenter2 dort **nicht** tut

Die Remote-Datenbank ist **kein Fernsteuerungskanal**. MHCenter2 liest von dort
